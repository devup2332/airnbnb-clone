...
const LoginModal = () => {
  ...
  // Strategies
  const { startOAuthFlow: googleStrategy } = useOAuth({ strategy: "oauth_google" })
  const { startOAuthFlow: facebookStrategy } = useOAuth({ strategy: "oauth_facebook" })
  const { startOAuthFlow: appleStrategy } = useOAuth({ strategy: "oauth_apple" })
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <TextInput autoCapitalize="none" onFocus={() => setStylesFocused({
        borderColor: "#000000",
        borderWidth: 2
      })} placeholder="Email" style={[defaultStyles.inputField, {
        marginBottom: 30,
        ...stylesFocused
      }]} />
      ...
      <View style={{ gap: 10 }}>
        {
          buttons.map(({ Icon, title, s }, index) => {
            return <TouchableOpacity style={styles.btnOutline} key={index} onPress={() => selectOAuth(s)}>
